...
def inter(a,b):
    ...
    inter=set([x for x in aset & bset])

    abar=aset.difference(inter)
    bbar=bset.difference(inter)
    # cbar=cset.difference(inter)

    inter_=np.array([x for x in aset & bset ])
    inter_=inter_.reshape((inter_.shape[0],3,16,9))

    for i,elt in enumerate([abar,bbar]):
        elt=np.array(list(elt))
        elt=elt.reshape((elt.shape[0],3,16,9))
