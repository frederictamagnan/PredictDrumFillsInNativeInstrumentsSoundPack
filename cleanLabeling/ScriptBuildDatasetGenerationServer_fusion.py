...
#     np.savez("./reduced_fills_plus_embeddings", fills=fills,genre=genre)
    ...
    genre=genre[1:]
    np.savez("./FillsExtractedFusion",vae=vae_array,genre=genre,track_array=track_array)
    return 0
...
def build_generation_dataset(p, npz):
    ...
    label_diff=dict(np.load(p + '/' + npz.replace('.','_diff.')))
    label = label['label']
    label_diff=label_diff['label']
    ...
    label=np.logical_and(label,label_diff)
