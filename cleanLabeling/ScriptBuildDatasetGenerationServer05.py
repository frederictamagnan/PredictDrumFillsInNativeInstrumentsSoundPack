...
#     np.savez("./reduced_fills_plus_embeddings", fills=fills,genre=genre)
    ...
    genre=genre[1:]
    np.savez("./FillsExtractedSupervised07",vae=vae_array,genre=genre,track_array=track_array)
    return 0
...
def build_generation_dataset(p, npz):
    ...
    label = label['label']
    metadata_dict = dict(np.load(p + '/' + npz.replace('_label07','_metadata_training')))
    vae=metadata_dict['vae_embeddings']
    # print(len(label),"LABEL",len(vae),"VAE")
    #     print(label.shape)
    # print(p + '/' + npz.replace('_label.npz', ''))
    multi = Multitrack(p + '/' + npz.replace('_label07', ''))
