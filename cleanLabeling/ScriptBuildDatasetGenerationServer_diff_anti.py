...
#     real_track_array=real_track_array[1:]
    ...
    genre=genre[1:]
    np.savez("./FillsExtractedDiffAnti",vae=vae_array,genre=genre,track_array=track_array)
    return 0
...
def build_generation_dataset(p, npz):
    ...
    label = label['label']
    metadata_dict = dict(np.load(p + '/' + npz.replace('_label_diff_anti','_metadata_training')))
    vae=metadata_dict['vae_embeddings']
    # print(len(label),"LABEL",len(vae),"VAE")
    #     print(label.shape)
    multi = Multitrack(p + '/' + npz.replace('_label_diff_anti', ''))
    ...
    indexes_fills_cleaned = allindices(string, '111')
