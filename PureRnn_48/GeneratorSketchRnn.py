...
class GeneratorSketchRnn:
    ...
    def generate(self,n,save=True):
        ...
        # X=encoder.encode_48(X)
        # X=X.reshape((X.shape[0],4,48,9))
        ...
        y=y_pred_cat.reshape((n,48, 9))
        # print(y[0])
        # print(X[1, 0, :, :],"value")
        X=encoder.encode_48(X)
        X = X.reshape((X.shape[0], 4, 48, 9))
        print(X.shape,y.shape)
        rp=X[:, 0:3, :, :].reshape((X.shape[0],48*3,9))
        ...
        new_dec=encoder.decode_48(new_dec)
    ...
    def generate_from(self,array,tag,save=True,th=0.5):
        n=len(array)
        self.load_model(batch_size=n)
        encoder=DrumReducerExpander()
        array_merge=array.reshape((n,4*96,9))
        array_merge_48=encoder.encode_48(array_merge)
        array_48=array_merge_48.reshape((n,4,48,9))
        X_dataset=SketchRnnDataset(numpy_array=array,inference=True,use_cuda=self.use_cuda)
        X_loader=torch.utils.data.DataLoader(dataset=X_dataset, batch_size=len(X_dataset),
                                                             shuffle=False,drop_last=True)
        with torch.no_grad():
            for i, (x) in enumerate(X_loader):
