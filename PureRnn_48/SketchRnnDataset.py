...
class SketchRnnDataset(data.Dataset):
    ...
    def __init__(self, numpy_array,use_cuda=False,inference=False):
        ...
        numpy_array=enc.encode_48(numpy_array)
        numpy_array=numpy_array.reshape((numpy_array.shape[0],4,48,9))

        X_previous=numpy_array[:,2,:,:]
        self.X=X_previous
        self.X=self.X.reshape((-1,48,9))

        if not inference:
            self.y=numpy_array[:,3,:,:]
            self.y=self.y.reshape((-1,48,9))
