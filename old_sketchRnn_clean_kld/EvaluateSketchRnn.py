...
class GeneratorSketchRnn:
    ...
    def classical_metrics(self):

        #reencode
        enc=DrumReducerExpander()
        y_pred=enc.decode_808(self.y_gen)
        y_test =enc.decode_808(self.y_or)
        y_pred=enc.encode_4(y_pred)
        y_test=enc.encode_4(y_test)
        y_test=y_test.reshape((-1))
        y_pred=y_pred.reshape(-1)



        # y_pred = self.y_gen.reshape((-1))
        # y_test = self.y_or.reshape((-1))
        print(y_pred.sum()/len(y_pred),y_test.sum()/len(y_test),"wholla")
...
for i_name,name in enumerate(['Clustering','Supervised','Diff']):
    ...
    for beta in [0.01,0.1,1,100,250]:
        ...
        t_prec.append(g.classical_metrics())
...
df_p = pd.DataFrame(row_tprec, columns = ['0.01','0.1','1','100','250'], index=['Clustering','Supervised','Diff'])

print("Original Dataframe" , df_w, sep='\n')
print("Original Dataframe" , df_t, sep='\n')
print("Original Dataframe" , df_p, sep='\n')
